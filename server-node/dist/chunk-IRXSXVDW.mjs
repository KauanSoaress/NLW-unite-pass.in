...
async function registerForEvent(app) {
  app.withTypeProvider().post("/events/:eventId/attendees", {
    ...
    const [event, amountOfAttendeesForEvent] = await Promise.all([
      // Maneira de realizar mais de 1 await de uma só vez, para não fazer multiplas buscas no BD, demorando mais
      ...
      prisma.attendee.count({
        // Get the amount of attendees for this event
        where: {
          eventId
        }
      })
    ...
    if (event?.maximumAttendees && amountOfAttendeesForEvent >= event?.maximumAttendees) {
      throw new BadRequest("The maximum number of attendeees for this event has been reached.");
    }
